<template>
  <div :class="classObj" class="app-wrapper">
    <div
      v-if="device === 'mobile' && opened && !isDev"
      class="drawer-bg"
    ...
    <Sidebar v-if="!isDev" class="sidebar-container" />
    <div :class="{ 'sidebar-content': isDev }" class="main-container">
      <!--Navbar-->
      <div v-if="!isDev" :class="{ 'fixed-header': fixedHeader, 'hide-header': hideHeader }">
    ...
    <RightDrawer v-if="showSetting">
      <!-- <div class="setting-item">
        <div class="setting-draw-title">文字按钮主题风格</div>

        <theme-pick @submit="submitForm" />
        <div class="divider" />
      </div> -->
      <div class="setting-item">
        <div class="setting-draw-title">背景主题风格</div>

        <theme-pick @submit="submitBackgroundForm" />
        <div class="divider" />
      </div>
      <div class="setting-item">
        <div class="setting-draw-title">内容区域</div>
        <content-area
          @toggleEdition="handleDevChange"
...
import { generateColors, writeNewStyle, writeNewBackgroundStyle, getStyleTemplate } from '@/utils'
...
export default defineComponent({
  ...
  setup() {
    ...
    const device = computed(() => store.getters['settingsModule/getDeviceState'])
    ...
    const handleDevChange = () => {
      // 改变state
      store.dispatch('settingsModule/toToggleDev')
    }
    ...
    // 	writeNewStyle('themeColor', originalStyle.value, colors)
    // }
    // /**
    //  * @description 切换背景主题色
    //  */
    const submitBackgroundForm = (primary: string) => {
      writeNewBackgroundStyle('themeBackgroundColor', primary)
    }
