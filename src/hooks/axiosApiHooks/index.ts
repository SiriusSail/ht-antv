...
export default function axiosApiHooks<P = any, D = any>(request: (params?: P) => Promise<{
  result: D;
}>)
...
// }
{
  ...
  //   result: {} as D
  // })
  function req({ initValue, params, needInit, deepCache, debounceInterval }: ApiHooksParams<P, D>) {
    ...
    const requsetData = reactive({
      ...
      request: (_data?: P) => {
        ...
        return request(_data || params).then((res) => {
