  ...
  <div class="MarketingMetrics">
    <div class="col_table">
      ...
      <MyTable border :data="tableData" :column="tableColumn" is-pagination :isLoading="isLoading" empty-text='暂无数据' :pagination="pagination" @paginationChange="handleCurrentChange" @paginationSizeChange='handleSizeChange'>
        ...
        <template #isExternalLink='{data}'>
          <span>{{data.row.isExternalLink==1?'是':'否'}}</span>
        </template>
    ...
    <el-dialog v-model="add_visible" :title="editType==='save'?'新增':'编辑' " @closed="handleClose">
      <MyForm ref="formRef" :form="queryData" status-icon :list="fromData" label-position="right" label-width="auto">
        <template #icon='{data}'>
          <el-input v-model="queryData[data.name]" :disabled='data.disabled' :placeholder='data.placeholder'> </el-input>
          <i :class="queryData[data.name]"></i>
          <el-popover :visible="modal" placement="left" width="50vw" title="请选择图标">
            <icons @success="onSuccess"></icons>
            <template #reference>
              <el-button type="primary" @click="handleClickChoose">选择图标</el-button>
            </template>
          </el-popover>
        </template>
        <template #submit>
...
export default defineComponent({
  ...
  setup () {
    ...
    const state = reactive<StateType>({
      ...
      fromSearchData: [
        {
          type: 'text', label: '菜单名称：', name: 'menuName', placeholder: '请输入流程名称', labelWidth: '90px',
          col: { span: 5 }
        },
        {
      ...
      queryData: { roleName: '', roleCode: '', isEnable: '0', remark: '', icon: '', isVisible: 1, routePath: '' },

      fromData: [
        {
          type: 'text', label: '菜单名称：', name: 'menuName', placeholder: '请输入菜单名称', labelWidth: '100px',
          col: { span: 12 },
          rules: { min: 2, max: 10, required: true, message: '请输入角色名称', trigger: 'blur' },
        },
        {
          type: 'text', label: '路由地址：', name: 'routePath', placeholder: '请输入路由地址', labelWidth: '100px',
          col: { span: 12 },
          rules: { min: 2, max: 10, required: true, message: '请输入路由地址', trigger: 'blur' },
        },
        {
        ...
        {
          type: 'select', label: '菜单类型：', name: 'menuType', placeholder: '请选择',
          col: { span: 12 },
          select: [{ value: 'M', label: "目录" }, { value: 'C', label: "菜单" }, { value: 'F', label: "按钮" }],
          rules: { required: true, message: '请输入英文名称', trigger: 'blur' },
        },
        {
          type: 'select', label: '是否显示：', name: 'isVisible', placeholder: '请选择',
          col: { span: 12 },
      ...
      tableColumn: [
        ...
        { label: '显示顺序', prop: 'orderNum', minWidth: '80px', showOverflowTooltip: true },
        { label: '父级编号', prop: 'parentId', minWidth: '80px', showOverflowTooltip: true },
        { label: '权限信息', prop: 'permission', minWidth: '80px', showOverflowTooltip: true },
        { label: '路由地址', prop: 'routePath', minWidth: '80px', showOverflowTooltip: true },
        { label: '组件编码', prop: 'componentCode', minWidth: '80px', showOverflowTooltip: true },
        { label: '组件路径', prop: 'componentPath', minWidth: '100px', showOverflowTooltip: true },
        { label: '记录更新人', prop: 'modifiedBy', minWidth: '100px', showOverflowTooltip: true },
        { label: '菜单图标', prop: 'icon', minWidth: '80px', showOverflowTooltip: true, slot: true },
        { label: '是否为外链', prop: 'isExternalLink', minWidth: '80px', showOverflowTooltip: true, slot: true },
        { label: '是否显示', prop: 'isVisible', minWidth: '80px', showOverflowTooltip: true, slot: true },
        ...
        { label: '操作', prop: 'operation', width: '160px', slot: true, fixed: 'right', }
      ],
      tableData: [],
      add_visible: false,
      isBtnLoading: false,
    ...
    const handleClose = () => {
      state.modal = false
      state.queryData = {
        roleName: '', roleCode: '', isEnable: '', remark: '', icon: '', isVisible: 1, routePath: ''
      }
