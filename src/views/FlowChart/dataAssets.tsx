...
export default defineComponent({
  setup() {
    const sideBarIndex = ref('2'); // 菜单 index
    ...
    const indassetDataList = apis.数据资产列表({
      needInit: true,
      initValue: []
    })
    ...
    return () => (
      <div className={styles.flow_chart}>
        ...
        <ElDialog
          v-model={ideaDialogFormVisible.value}
          title='修改数据资产'
          width='75%'
          v-slots={{
        ...
        >
          <ElForm model={form.value} ref={(e: any) => {
            ...
            <ElFormItem label='数据资产'>
              <ElSelect
                v-model={form.value.tableChName}
                placeholder='请选择数据资产表'
                autocomplete='off'
              >
            ...
            <ElFormItem label='数据资产备注'>
              <ElInput
                type='textarea'
                v-model={form.value.description}
                placeholder='请输入数据资产备注'
                autocomplete='off'
              />
            </ElFormItem>
            <ElTable
