<template>
  <div class="MyMission">
    ...
    <div class="myMission-header">
      <el-row :gutter="20">
        <el-col :span="4">
          <el-select size="small" v-model="pagination.orderField" filterable placeholder="全部" @change="handleTypeChage">
            <el-option v-for="item in options1" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-col>
        <el-col :span="4">
          <el-select size="small" v-model="pagination.order" filterable placeholder="全部" @change="handleOrderChage">
            <el-option v-for="item in options3" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-col>
        <el-col :span="4">
          <el-select size="small" v-model="pagination.demandOrTask" filterable placeholder="全部" @change="handleTypesChage">
            <el-option v-for="item in options2" :key="item.value" :label="item.label" :value="item.value" :value-key="item.value" />
          </el-select>
        </el-col>
        <el-col :span="6">
          <el-input size="small" v-model="pagination.demandName" @input="changeInput" placeholder="搜索或过滤结果">
            <template #prefix>
        ...
        <el-button size="small" type="primary" @click='onRefresh' :loading='isBtnLoading'>刷新</el-button>
      </el-row>
    </div>
    <div class="MyMission-body">
      <el-row :gutter="10">
        <el-col :span="4">
          <DraggableList ref='refOneItem' color="#FFF1DB" label="待开始" :num="orderNum.one" :types='1' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination'>
            <div :ref="oneRef" class="list" :types='1'>
              <DraggableItem :types='1' v-for="item in listOne" :item="item" :key="item.id" class="item" color="#FFBA48" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
        <el-col :span="4">
          <DraggableList ref='refTwoItem' color="#E0EDFF" label="进行中" :num="orderNum.two" :types='2' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination1'>
            <div :ref="twoRef" class="list" :types='2'>
              <DraggableItem :types='2' v-for="item in listTwo" :item="item" :key="item.id" class="item" color="#C5DEFF" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
        <el-col :span="4">
          <DraggableList ref='refThreeItem' color="#E5EAF0" label="已完成" :num="orderNum.three" :types='3' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination2'>
            <div :ref="threeRef" class="list" :types='3'>
              <DraggableItem :types='3' v-for="item in listThree" :item="item" :key="item.id" class="item" color="#E0EAF2" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
        <el-col :span="4">
          <DraggableList ref='refFourItem' color="#D1F8E8" label="已验收" :num="orderNum.four" :types='4' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination3'>
            <div :ref="fourRef" class="list" :types='4'>
              <DraggableItem :types='4' v-for="item in listFour" :item="item" :key="item.id" class="item" color="#5FE1A9" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
        <el-col :span="4">
          <DraggableList ref='refFiveItem' color="#FFE8E3" label="已拒绝" :num="orderNum.five" :types='5' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination4'>
            <div :ref="fiveRef" class="list" :types='5'>
              <DraggableItem :types='5' v-for="item in listFive" :item="item" :key="item.id" class="item" color="#FFAA97" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
        <el-col :span="4">
          <DraggableList ref='refSixItem' color="#FFF8E3" label="待确定" :num="orderNum.six" :types='6' @eventPre="handlePre" @eventNext="handleNext" :pages='pagination5'>
            <div :ref="sixRef" class="list" :types='6'>
              <DraggableItem :types='6' v-for="item in listSix" :item="item" :key="item.id" class="item" color="#FFE083" @eventItem='handleItem' />
            </div>
          </DraggableList>
        </el-col>
      </el-row>
    ...
    <el-dialog top="20px" custom-class="kanban-dialog" v-model="dialogVisible" title="需求详情" width="95%" @opened="opened" @close="close">
      <div class="dialog-con">
        <div class="c-head">
          <el-row :gutter="20">
            <el-col :span="18">
              <el-steps :active="4" align-center>
                <el-step title="2021-01-11">
                  <template #icon>规</template>
                </el-step>
                <el-step title="2021-01-12"></el-step>
                <el-step title="2021-01-13"></el-step>
                <el-step title="2021-01-14">
                  <template #icon>
              ...
              <div class="form">
                <addDemand ref='refDemand' :isLoading='isLoading' @eventSubmit='handleSubmit' @reightData='getReightData' />
              </div>
            </el-col>
            <el-col :span="6">
              <div class="feasibility">
                <!-- <Echart ref="echartRef" :init="false" :option="option"></Echart> -->
                敬请期待
              </div>
              <div class="info">
                <el-button type="primary" size="mini" @click="handleShowTask">{{isShowTack?'取消':'开始'}}任务分解</el-button>
                <el-button class="btn" size='mini' type="primary" @click="handleLead">导出</el-button>
                <div class="devInformation">
                  <template v-if="isShowTack">
                    <MyForm ref="formRef" :form="queryData" status-icon :list="fromData" label-position="right" label-width="auto">
                      <template #submit>
                        <div class="operation">
                          <el-button type="primary" size="small" @click='handleTaskSubmit'>保存</el-button>
                        </div>
                      </template>
                    </MyForm>
                  </template>
                ...
                <div class="col_task">
                  <template v-if="taskData.length>0">
                    <MyTable border :data="taskData" :column="taskTableColumn"></MyTable>
                  </template>
                </div>

                <template v-if="reightData.length>0">
                  <MyTable border :data="reightData" :column="tableColumn"></MyTable>
                </template>
    ...
    <!-- 任务详情 -->
    <el-dialog top="20px" custom-class="kanban-dialog" v-model="dialogTask" title="任务详情" width="95%">
      <div class="dialog-con">
        <el-row :gutter="20">
          <el-col :span="22">
            <MyTable border :data="taskData" :column="taskTableColumn"></MyTable>
          ...
          <el-col :span="2">
            <el-button type="primary" size="small" @click='handleTarget'>指标开发</el-button>
          </el-col>
        </el-row>
        <addDemand ref='refDemand' :isLoading='isLoading' @eventSubmit='handleSubmit' @reightData='getReightData' />
      </div>
    ...
    <!-- 新增 -->
    <el-dialog top="20px" v-model="addDialogVisible" title="创建需求" width="95%">
      <addDemand ref='refDemandTemplate' :isLoading='isLoading' @eventSubmit='handleSubmit' />
    </el-dialog>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  components: { DraggableList, DraggableItem, Echart, addDemand, MyForm, MyTable },
  setup () {
    /****************************************** 初始化数据 **************************************************/
    ...
    const state = reactive<stateType>({
      ...
      demandState: '',
      detailTiem: {},
      queryData: { demandDeveloper: '', demandDeveloperId: '', demandName: '', demandVersion: '', parentId: '' },
      fromData: [
        {
          type: 'select', label: '任务开发人', name: 'demandDeveloper', placeholder: '请选择开发人名称', select: selectList, labelWidth: '100px',
          rules: { required: true, message: '请选择开发人名称', trigger: 'blur', },
        },
        {
          type: 'text', label: '任务名称', name: 'demandName', placeholder: '请输入任务名称', labelWidth: '100px',
          rules: { required: true, message: '请输入任务名称', trigger: 'blur', },
        },
        {
          type: 'text', label: '任务版本', name: 'demandVersion', placeholder: '请输入任务版本', labelWidth: '100px',
      ...
      options3: [{ value: "desc", label: "降序" }, { value: "asc", label: "升序" }],
      pagination: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      pagination1: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      pagination2: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      pagination3: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      pagination4: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      pagination5: { ...comState.pagination, orderField: 'gmt_create', demandOrTask: '', demandName: '', order: 'desc' },
      listOne: [],
      listTwo: [],
      listThree: [],
      listFour: [],
      ...
      tableColumn: [
        ...
        { label: '自查结论', prop: 'selfCheckContent' },
        { label: '整改事项描述', prop: 'rectificatExplain' }
      ],
      taskTableColumn: [
        { label: '任务开发人', prop: 'demandDeveloper' },
        { label: '任务名称', prop: 'demandName' },
        { label: '任务版本', prop: 'demandVersion' }
      ],
      dialogVisible: false,
      isLoading: false,
      dialogTask: false,
    ...
    const oneRef = (e: any) => { oneElList.value.push(e) };
    const twoRef = (e: any) => { twoElList.value.push(e) };
    const threeRef = (e: any) => { threeElList.value.push(e) };
    const fourRef = (e: any) => { fourElList.value.push(e) };
    const fiveRef = (e: any) => { fiveElList.value.push(e) };
    const sixRef = (e: any) => { sixElList.value.push(e) };
    ...
    // //获取搜索值
    // const searchWord = computed(() => store.state.demandModule.searchWord);
    // //监听用户是否有操作
    // watch(searchWord, () => {
    //   state.pagination.demandName = searchWord
    //   initData()
    // })
    ...
    const handleItem = async (props: any) => {
      ...
      if (props.item.parentId) {
        ...
        refDemand.value?.update(res.result, { demandName: props.item.demandName, reviewState: 100 }, 'edit')
      } else {
        state.dialogVisible = true
        state.isLoading = true
        const res = await Service.detail({ demandId: props.item.rowId })
        refDemand.value?.update(res.result, { demandName: props.item.demandName, reviewState: props.item.reviewState }, 'edit')
    ...
    const handleLead = async () => {
      dwonloadFile(1, `${import.meta.env.VITE_APP_BASE_URL}/ddm-iic/dem/demand/exportExcel?demandId=${state.detailTiem.rowId}&demandName=${state.detailTiem.demandName}`)
    }
    ...
    const getRightList = async (demandState?: number) => {
      const res = await Service.getList(delField({ demandOrTask: 2, demandState: demandState, orderField: 'gmt_create', order: 'desc', ...comState.pagination }))
      state.taskData = res.result.records
    }
    return {
      refOneItem,
