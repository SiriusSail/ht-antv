    ...
    <el-dialog v-model="add_visible" :title="editType==='save'?'新增':'编辑' " @closed="handleClose">
      <MyForm ref="formRef" :form="queryData" status-icon :list="fromData" label-position="right" label-width="auto">
        <template #menuIds='{data}'>
          <el-transfer :titles="['全部菜单', '已授权菜单']" v-model="queryData[data.name]" :props="{key: 'rowId',label: 'menuName'}" :data="menuList">
          </el-transfer>
        </template>
        <template #submit>
          <div class="operation">
...
export default defineComponent({
  ...
  setup () {
    ...
    const state = reactive<StateType>({
      sId: '',
      querySearchData: { orderField: 'gmt_create', order: 'desc', roleName: '', },
      fromSearchData: [
        {
          type: 'text', label: '角色名称：', name: 'roleName', placeholder: '请输入流程名称', labelWidth: '90px',
          col: { span: 5 }
        },
        {
      ...
      queryData: { roleName: '', roleCode: '', isEnable: 0, remark: '', menuIds: [] },

      fromData: [
        {
          type: 'text', label: '角色名称', name: 'roleName', placeholder: '请输入角色名称', labelWidth: '90px',
          col: { span: 12 },
          rules: { required: true, message: '请输入角色名称', trigger: 'blur' },
        },
        {
          type: 'text', label: '英文名称', name: 'roleCode', placeholder: '请输入英文名称', labelWidth: '90px',
          col: { span: 12 },
          rules: { required: true, message: '请输入英文名称', trigger: 'blur' },
        },
        {
          type: 'radioGroup', label: '是否可用', name: 'isEnable', placeholder: '请选择',
      ...
      tableColumn: [
        { label: '角色名称', prop: 'roleName', minWidth: '80px', showOverflowTooltip: true },
        { label: '英文名称', prop: 'roleCode', minWidth: '100px', showOverflowTooltip: true },
        { label: '记录更新人', prop: 'modifiedBy', minWidth: '100px', showOverflowTooltip: true },
        { label: '是否可用', prop: 'isEnable', minWidth: '80px', showOverflowTooltip: true, slot: true },
        { label: '记录创建人', prop: 'createBy', minWidth: '100px', showOverflowTooltip: true },
        { label: '更新时间', prop: 'gmtModified', showOverflowTooltip: true },
        { label: '创建时间', prop: 'gmtCreate', showOverflowTooltip: true },
    ...
    const handleClose = () => {
      state.queryData = {
        roleName: '', roleCode: '', isEnable: 0, remark: '', menuIds: []
      }
