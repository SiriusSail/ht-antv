...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let isCloseEvente: 'change' | 'watch' | '' = '';
    let jsonEditor: CodeMirror.Editor;
    ...
    const initCodeMirror = () => {
      ...
      jsonEditor.on('change', (cm: { getValue: () => any }) => {
        if (isCloseEvente === 'watch') {
          isCloseEvente = '';
          return
        }
        isCloseEvente = 'change';
    ...
    watchEffect(() => {
      console.log(props.modelValue); // 不能删除
      if (jsonEditor) {
        if (isCloseEvente === 'change') {
          isCloseEvente = '';
          return
        }
        isCloseEvente = 'watch';
